...
namespace vtk
  ...
  void vtkFormsWindowControl::OnResize( System::EventArgs^ ea)
    {
#ifdef TRACE
    m_traceSource->TraceEvent(TraceEventType::Information,0,"OnResize begin");
#endif // TRACE
  ...
  void vtkFormsWindowControl::OnMouseMove( System::Windows::Forms::MouseEventArgs^ ea )
    ...
    static_cast<::vtkWin32RenderWindowInteractor*>(this->GetInteractorNative())->
      OnMouseMove((HWND) this->Handle.ToPointer(), nFlags, x, y);
  ...
  void vtkFormsWindowControl::ShowSplashScreen()
    ...
    ::vtkRendererCollection* renderers = this->GetRenderWindowNative()->GetRenderers();
    renderers->InitTraversal();
    ::vtkRenderer* ren = renderers->GetFirstRenderer();
    if ( 0 != ren )
      {
