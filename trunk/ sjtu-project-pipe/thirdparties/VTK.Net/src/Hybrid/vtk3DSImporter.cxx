...
static void parse_texture_coordinate_array (vtk3DSImporter *importer, vtk3DSMesh *mesh);
...
void vtk3DSImporter::ImportActors (vtkRenderer *renderer)
{
  ...
  // Delete the actor collection before use it
  if (NULL != ActorCollection)
  {
  ...
  for (mesh = this->MeshList; mesh != (vtk3DSMesh *) NULL; 
	...
	if (strlen(material->tex_map) != 0)
	{
        // The texture map is a relative file name;
        // We should format a absolute file name
        char sz[256];
        strcpy(sz, FileName);
        char* pEnd = strrchr(sz, '\\');
        if (pEnd != NULL)
        {
...
vtkPolyData *vtk3DSImporter::GeneratePolyData (vtk3DSMesh *mesh)
{
  ...
  if (material)
  {
	  UScale = material->scale[0];
	  VScale = material->scale[1];

	  UOffset = material->offset[0];
	  VOffset = material->offset[1];
  }
  ...
  {
	  textureCoordinates = vtkFloatArray::New();
	  textureCoordinates->SetNumberOfComponents(2);
	  textureCoordinates->Allocate(mesh->textureCoords*2);
	  textureCoordinates->SetName("TextureCoordinates");

	  for (i = 0; i < mesh->textureCoords; i++)
	  {
		  mesh->textCoords[i][0] = (mesh->textCoords[i][0]  + UOffset) * UScale;
		  mesh->textCoords[i][1] = (mesh->textCoords[i][1]  + UOffset) * UScale;
		  textureCoordinates->InsertTuple(i, mesh->textCoords[i]);
	  }

	  //textureCoordinates->setcom
    ...
    if (chunk.end <= mainchunk->end) 
      ...
      switch (chunk.tag) 
        ...
        case 0xA200: //mprop->tex_strength = parse_percentage(importer);
		  parse_matTextureMap(importer, &chunk, mprop);
...
static void parse_matTextureMap (vtk3DSImporter *importer, vtk3DSChunk *mainchunk, vtk3DSMatProp* textmap)
{
  vtk3DSChunk chunk;
  ...
  do  
    ...
    if (chunk.end <= mainchunk->end) 
      ...
      switch (chunk.tag) 
        {
	  case 0x0030: textmap->tex_strength  = parse_int_percentage(importer)/100.0;
		  break;

	  case 0x0031: textmap->tex_strength = parse_float_percentage(importer);
		  break;

	  case 0xA300: strcpy (textmap->tex_map, read_string(importer));
		  break;

	  case 0xA354 : textmap->scale[0] = read_float(importer);
		  break;

	  case 0xA356: textmap->scale[1] = read_float(importer);
		  break;

	  case 0xA358 : textmap->offset[0] = read_float(importer);
		  break;

	  case 0xA35A: textmap->offset[1] = read_float(importer);
		  break;
      }
	}
    ...
    if (chunk.end <= mainchunk->end) 
      ...
      switch (chunk.tag) 
        ...
        case 0x4160: parse_mesh_matrix(importer, mesh);
          break;
		case 0x4140: parse_texture_coordinate_array(importer, mesh);
			break;
	  }
      }
...
static void parse_texture_coordinate_array(vtk3DSImporter *importer, vtk3DSMesh *mesh)
{
  ...
  mesh->textureCoords = read_word(importer);
  if (mesh->textureCoords)
  {
	  mesh->textCoords = (vtk3DSTextCoord *) malloc (mesh->textureCoords * sizeof(*(mesh->textCoords)));
	  for (i = 0; i < mesh->textureCoords; i++)
	  {
		  mesh->textCoords[i][0] = read_float(importer)/* * 0.05*/; // 0.05 should be read.
		  mesh->textCoords[i][1] = -read_float(importer)/* * 0.05*/;

		  //printf("%d u = %lf, v = %lf\n", i + 1, mesh->textCoords[i][0], mesh->textCoords[i][1]);

		  if (mesh->textCoords[i][0] < -3.402823466e+38F / 4.0f)
		  {
			  mesh->textCoords[i][0] = 0.0f;
			  mesh->textCoords[i][1] = 0.0f;
		  }
	  }
  }
}
